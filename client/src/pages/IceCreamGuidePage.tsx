...
import {
  ...
  ICE_CREAM_QUALITY_TIER_CONFIG,
  getIceCreamColor,
  type IceCreamEntry,
  type IceCreamQualityTier,
} from "@/lib/iceCreamDatabase";
import { X, ChevronDown, ChevronUp, IceCreamCone, Info, CheckCircle, AlertTriangle, ArrowLeft } from "lucide-react";
import { GuideHero } from "@/components/GuideHero";

const TIER_ORDER: IceCreamQualityTier[] = ["elite", "good", "average", "poor", "avoid"];

function ScoreRing({ score, size = 56 }: { score: number; size?: number }) {
  const color = getIceCreamColor(score);
  const r = (size - 8) / 2;
...
function IceCreamCard({ item, onClick }: { item: IceCreamEntry; onClick: () => void }) {
  const tier = ICE_CREAM_QUALITY_TIER_CONFIG[item.qualityTier];
  return (
    <button
      onClick={onClick}
    ...
    >
      <div className="flex items-start gap-3">
        <ProductImage barcode={item.barcode} name={item.name} brand={item.brand} imageUrl={item.imageUrl} category="Ice Cream" size={52} />
        <ScoreRing score={item.score} size={52} />
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            <div>
              <p className="font-semibold text-stone-800 dark:text-stone-100 text-sm leading-tight line-clamp-2">{item.name}</p>
          ...
          <div className="flex flex-wrap gap-1 mt-2">
            ...
            {item.hasCarrageenan && (
              <span className="text-xs bg-orange-50 text-orange-700 border border-orange-200 rounded-full px-2 py-0.5">⚠ Carrageenan</span>
            )}
            {item.hasArtificialSweeteners && (
              <span className="text-xs bg-purple-50 text-purple-700 border border-purple-200 rounded-full px-2 py-0.5">Sugar Alcohols</span>
            )}
          </div>
        </div>
      </div>
...
function IceCreamModal({ item, onClose }: { item: IceCreamEntry; onClose: () => void }) {
  const tier = ICE_CREAM_QUALITY_TIER_CONFIG[item.qualityTier];
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/40 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        ...
        <div className="w-full h-44 bg-stone-50 flex items-center justify-center overflow-hidden">
          <ProductImage barcode={item.barcode} name={item.name} brand={item.brand} imageUrl={item.imageUrl} category="Ice Cream" size={160} className="!w-full !h-full !rounded-none" />
        </div>
        {/* Header */}
        ...
        <div className="overflow-y-auto flex-1 p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            {[
              ...
              { label: "Seed Oils", value: item.hasSeedOils, inverted: true },
              { label: "Carrageenan", value: item.hasCarrageenan, inverted: true },
              { label: "Artificial Sweeteners", value: item.hasArtificialSweeteners, inverted: true },
            ].map((attr) => (
          ...
          <div className="bg-blue-50 rounded-xl p-3 text-sm">
            <span className="font-semibold text-blue-800">Ingredient Count: </span>
            <span className="text-blue-700">{item.ingredientCount} ingredients</span>
            {item.ingredientCount <= 5 && (
              <p className="text-blue-600 text-xs mt-1">Excellent — 5 or fewer ingredients means minimal processing.</p>
            )}
            {item.ingredientCount > 15 && (
              <p className="text-orange-600 text-xs mt-1">High ingredient count suggests heavy processing and additives.</p>
            )}
          </div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {item.nutrients.calories !== undefined && (
                  <div className="bg-stone-50 rounded-xl p-2 text-center">
                    ...
                    <p className="font-bold text-stone-800 dark:text-stone-100">{item.nutrients.calories}</p>
                  </div>
                )}
                {item.nutrients.fat !== undefined && (
                  <div className="bg-stone-50 rounded-xl p-2 text-center">
                    <p className="text-xs text-stone-500 dark:text-stone-400">Fat</p>
                    <p className="font-bold text-stone-800 dark:text-stone-100">{item.nutrients.fat}g</p>
                  </div>
                )}
                {item.nutrients.sugar !== undefined && (
                  <div className="bg-stone-50 rounded-xl p-2 text-center">
                    <p className="text-xs text-stone-500 dark:text-stone-400">Sugar</p>
                    <p className="font-bold text-stone-800 dark:text-stone-100">{item.nutrients.sugar}g</p>
                  </div>
                )}
                {item.nutrients.protein !== undefined && (
                  <div className="bg-stone-50 rounded-xl p-2 text-center">
                    <p className="text-xs text-stone-500 dark:text-stone-400">Protein</p>
                    <p className="font-bold text-stone-800 dark:text-stone-100">{item.nutrients.protein}g</p>
                  </div>
                )}
                {item.nutrients.servingSize && (
                  <div className="bg-stone-50 rounded-xl p-2 text-center col-span-2">
                    <p className="text-xs text-stone-500 dark:text-stone-400">Serving</p>
                    <p className="font-bold text-stone-800 dark:text-stone-100">{item.nutrients.servingSize}</p>
                  </div>
                )}
              </div>
            </div>
...
export default function IceCreamGuidePage() {
  const [, setLocation] = useLocation();
  const [search, setSearch] = useState("");
  const [activeCategory, setActiveCategory] = useState("all");
  const [activeTier, setActiveTier] = useState<IceCreamQualityTier | "all">("all");
  const [selected, setSelected] = useState<IceCreamEntry | null>(null);
  const [expandedTiers, setExpandedTiers] = useState<Set<string>>(new Set(["elite", "good"]));
  const [priceRange, setPriceRange] = useState<PriceRange>("all");

  const filtered = ALL_ICE_CREAM.filter((item) => {
    const matchSearch =
      !search ||
  ...
  const byTier = TIER_ORDER.reduce<Record<string, IceCreamEntry[]>>((acc, t) => {
  ...
  return (
    <div className="ec-page-bg">
      <GuideHero imageUrl="/manus-storage/icecream-category_5814d61f.jpg" />
      {/* Header */}
      <div className="ec-sticky-header sticky top-0 z-30">
        <div className="max-w-2xl mx-auto px-4 py-4">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="w-10 h-10 rounded-2xl bg-pink-50 flex items-center justify-center">
              <IceCreamCone size={20} className="text-pink-600" />
            </div>
            <div>
              <h1 className="text-xl font-bold text-stone-900">Ice Cream Guide</h1>
              <p className="text-xs text-stone-500 dark:text-stone-400">{ALL_ICE_CREAM.length} products · Ingredient quality, seed oils, carrageenan ratings</p>
            </div>
          </div>
          {/* Search */}
          <input
            type="text"
            placeholder="Search ice cream brands..."
            value={search}
        ...
        <div className="max-w-2xl mx-auto px-4 pb-3">
          <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
            ...
            {TIER_ORDER.map((t) => {
              const cfg = ICE_CREAM_QUALITY_TIER_CONFIG[t];
              return (
                <button
                  key={t}
      ...
      <div className="max-w-2xl mx-auto px-4 pt-4">
        <div className="bg-blue-50 border border-blue-200 rounded-2xl p-3 flex gap-2">
          ...
          <p className="text-xs text-blue-700 leading-relaxed">
            <strong>Best ice cream</strong> has 5 or fewer ingredients: cream, milk, sugar, eggs, and flavor. Watch for <strong>carrageenan</strong> (gut irritant), <strong>palm oil</strong> (cheap filler), and <strong>polysorbate 80</strong> (emulsifier linked to inflammation). Grass-fed dairy has more omega-3 and CLA.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto px-4 py-4 space-y-4 pb-28">
        {TIER_ORDER.map((tier) => {
          ...
          const cfg = ICE_CREAM_QUALITY_TIER_CONFIG[tier];
            ...
            <div key={tier} className="rounded-2xl overflow-hidden border" style={{ borderColor: cfg.border }}>
              ...
              {isExpanded && (
                <div className="grid grid-cols-1 gap-2 p-3 bg-white dark:bg-stone-800">
                  {items.map((item) => (
                    <IceCreamCard key={item.id} item={item} onClick={() => setSelected(item)} />
                  ))}
                </div>
              )}
            </div>
        ...
        {filtered.length === 0 && (
          <div className="text-center py-16">
            <IceCreamCone size={40} className="mx-auto text-stone-300 mb-3" />
      ...
      {selected && <IceCreamModal item={selected} onClose={() => setSelected(null)} />}
    </div>
  );
}
